...
| -------------------- | --------------------------------------------------------------------------- | ----------------------------- |
| $\text{Bitwise}$     | $\sim A,\ +A,\ -A$ <br> $A \& B,\ A \mid B,\ A \oplus B,\ A\ \hat{}\sim\ B$ | $L(A)$ <br> $\max(L(A),L(B))$ |
| $\text{Logical}$     | $!A,\ A \&\& B,\ A \mid\mid B$                                              | $1\ \text{bit}$               |
| $\text{Reduction}$   | $\&A,\ \sim\&A,\ \mid A,\ \sim\mid A,\ \hat{}A,\ \sim\hat{}A$               | $1\ \text{bit}$               |
...
> [!question] How does Verilog knows the order of logical operations
